...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // Artists data with growth trends
      supabase
        .from('artists')
      ...
      // All artists for reputation distribution
      supabase
        .from('artists')
    ...
    const growthTrends = generateGrowthTrends(
      artistsData.data || [],
      fansData.data || [],
      campaignsData.data || [],
      daysBack
    ...
    // Generate reputation distribution
...
function generateGrowthTrends(artists: any[], fans: any[], campaigns: any[], daysBack: number) {
  ...
  for (let i = daysBack - 1; i >= 0; i--) {
    ...
    const artistsOnDate = artists.filter(a => 
      new Date(a.created_at).toISOString().split('T')[0] <= dateStr
    ).length;
    
    const fansOnDate = fans.filter(f => 
      new Date(f.created_at).toISOString().split('T')[0] <= dateStr
    ).length;
    
    const campaignsOnDate = campaigns.filter(c => 
...
function generateEngagementTrends(emails: any[], daysBack: number) {
  ...
  for (let i = daysBack - 1; i >= 0; i--) {
    ...
    const dayEmails = emails.filter(e => 
      new Date(e.sent_at).toISOString().split('T')[0] === dateStr
    );
