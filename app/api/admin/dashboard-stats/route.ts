...
export async function GET() {
  try {
    // Only super admins can access dashboard stats
    ...
    const activeArtists = artists.filter(artist => 
      new Date(artist.updated_at) > thirtyDaysAgo
    );
    ...
    const reputationStats = {
      artists_with_good_reputation: reputationData.filter(a => 
        (a.reputation_score || 100) >= 80 && !a.sending_suspended
      ).length,
      artists_with_warnings: reputationData.filter(a => 
        (a.reputation_score || 100) >= 60 && (a.reputation_score || 100) < 80 && !a.sending_suspended
      ).length,
