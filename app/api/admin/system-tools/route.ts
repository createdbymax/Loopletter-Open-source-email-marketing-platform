...
export async function POST(request: NextRequest) {
  try {
    // Only super admins can use system tools
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to execute system tool' },
      { status: 500 }
    );
  }
}
...
async function updateEngagementScores() {
  try {
    // This would be a complex operation to recalculate engagement scores
...
async function updateReputationScores() {
  try {
    // Get all artists and recalculate their reputation scores
    ...
    for (const artist of artists || []) {
      // This would call the reputation calculation function
