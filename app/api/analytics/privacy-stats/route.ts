...
export async function GET(req: NextRequest) {
    try {
        ...
        const openTrackingAllowed = fans.filter(fan => fan.tracking_preferences?.allow_open_tracking !== false).length;
        const clickTrackingAllowed = fans.filter(fan => fan.tracking_preferences?.allow_click_tracking !== false).length;
        return NextResponse.json({
