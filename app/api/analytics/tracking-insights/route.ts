...
export async function GET(req: NextRequest) {
  try {
    ...
    // Process campaign data to calculate tracking metrics
    const insights = campaigns.map(campaign => {
      ...
      // Calculate unique opens/clicks (first occurrence per email)
      const uniqueOpens = new Set(
        emails.filter(e => e.opened_at).map(e => e.id)
      ).size;
      const uniqueClicks = new Set(
        emails.filter(e => e.clicked_at).map(e => e.id)
      ).size;
