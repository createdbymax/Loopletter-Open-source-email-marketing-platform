...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!data.musicLinks.some(link => link.trim()) || !data.genre) {
            return NextResponse.json({ error: 'At least one music link and genre are required' }, { status: 400 });
        }
        if (data.currentReachMethods.length === 0 || data.mainGoals.length === 0) {
            return NextResponse.json({ error: 'Current reach methods and main goals are required' }, { status: 400 });
        }
        ...
        if (existingEntry) {
            return NextResponse.json({ error: 'Email already registered for early access' }, { status: 409 });
        }
...
export async function GET() {
    try {
        ...
        if (error) {
            ...
            return NextResponse.json({ error: 'Failed to fetch early access data' }, { status: 500 });
        }
