...
import { 
  validateEmailForSpam, 
  checkRateLimits, 
  validateBulkImport,
  recordConsentVerification,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if artist is suspended from sending
    ...
    // For bulk imports, use the bulk validation
    ...
    // Process each fan with spam prevention
    for (const fanData of fansData) {
      ...
      if (spamCheck.action === 'quarantine') {
        try {
          ...
          continue; // Don't add to addedFans since it's quarantined
        } catch (error) {
      ...
      // Add warnings for moderate-risk emails that were approved
      ...
      try {
        ...
        // Record consent verification if provided
        ...
        // Log successful addition for compliance
