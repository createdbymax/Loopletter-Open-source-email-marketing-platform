...
export async function POST(request: NextRequest) {
  try {
    ...
    // Provide compliance guidance based on results
    const complianceGuidance = [];
    
    if (validation.requiresDoubleOptIn.length > 0) {
      complianceGuidance.push(
        `${validation.requiresDoubleOptIn.length} emails require double opt-in confirmation to comply with anti-spam regulations.`
      );
    }

    if (source === 'import') {
      complianceGuidance.push(
        'For imported email lists, you must have documented consent from each subscriber. ' +
    ...
    if (validation.invalid.length > validation.valid.length * 0.1) {
      complianceGuidance.push(
        'High percentage of invalid emails detected. This may indicate a purchased or scraped list, ' +
    ...
    return NextResponse.json(
      { error: 'Failed to validate bulk import' },
      { status: 500 }
    );
  }
}