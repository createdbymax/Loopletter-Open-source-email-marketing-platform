...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!priceId) {
            return NextResponse.json({ error: 'Price ID not configured for selected plan' }, { status: 400 });
        }
        const artist = await getOrCreateArtistByClerkId(userId, '', '');
        const session = await createCheckoutSession(artist, priceId, successUrl || `${req.headers.get('origin')}/dashboard/settings?tab=subscription&success=true`, cancelUrl || `${req.headers.get('origin')}/dashboard/settings?tab=subscription&canceled=true`);
