...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!ALLOWED_TYPES.includes(file.type)) {
            ...
            return NextResponse.json({ error: 'Invalid file type. Only JPEG, PNG, and WebP files are allowed.' }, { status: 400 });
        }
        ...
        const fileName = `email-images/${user.id}/${timestamp}.jpg`;
        ...
        try {
            ...
            return NextResponse.json({ error: 'Failed to process image. Please ensure it\'s a valid image file.' }, { status: 400 });
        }
