      ...
      <Card>
        ...
        <CardContent>
          {loading ? (<div className="flex justify-center py-8">
            ...
            </div>) : error ? (<div className="text-center py-8 text-red-500">{error}</div>) : advancedData ? (<Tabs defaultValue="locations">
                ...
                <div className="space-y-4">
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={advancedData.locations} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                    {advancedData.locations.map((location: {
                city: string;
                country: string;
                count: number;
            }, index: number) => (<div key={index} className="bg-gray-50 p-3 rounded-lg">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={advancedData.devices} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="count" label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`}>
                            {advancedData.devices.map((entry: {
                type: string;
                count: number;
                percentage: number;
            }, index: number) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                          </Pie>
                          <Tooltip />
                        </PieChart>
                      </ResponsiveContainer>
                    ...
                    <div className="space-y-3">
                      {advancedData.devices.map((device: any, index: number) => (<div key={index} className="flex items-center justify-between">
              ...
              <TabsContent value="timing">
                <div className="space-y-4">
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={advancedData.timeOfDay} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3"/>
                        <XAxis dataKey="hour"/>
                        <YAxis />
                        <Tooltip />
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={advancedData.emailClients} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="count" label={({ name, percent }) => `${name} ${((percent ?? 0) * 100).toFixed(0)}%`}>
                            {advancedData.emailClients.map((entry: {
                name: string;
                count: number;
            }, index: number) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                          </Pie>
                    ...
                    <div className="space-y-3">
                      {advancedData.emailClients.map((client: {
                name: string;
                count: number;
                percentage: number;
            }, index: number) => (<div key={index} className="flex items-center justify-between">
                          <div className="flex items-center">
                            <div className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: COLORS[index % COLORS.length] }}></div>
