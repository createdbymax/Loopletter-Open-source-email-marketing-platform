...
import { Zap, Plus, Trash2, Edit, Mail, Tag, Clock, Calendar, Lock, AlertTriangle } from 'lucide-react';
...
export function AutomationManager({ artist }: AutomationManagerProps) {
    ...
    const toggleAutomationStatus = async (id: string, currentStatus: string) => {
        try {
            ...
            setAutomations(automations.map(automation => automation.id === id ? { ...automation, status: newStatus } : automation));
        }
        catch (error) {
    ...
    return (<FeatureGate feature="automations" artist={artist} fallback={<div className="space-y-6">
        ...
        {loading ? (<div className="flex justify-center py-12">
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {automations.map((automation) => (<TableRow key={automation.id}>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          <Zap className="h-4 w-4 text-blue-600"/>
                          {automation.name}
                        </div>
                        {automation.description && (<div className="text-xs text-gray-500 mt-1">
                      ...
                      <TableCell>
                        ...
                        {automation.trigger.delay && (<div className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          {automation.actions.map((action, i) => (<div key={i} className="flex items-center gap-1 text-sm">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Switch checked={automation.status === 'active'} onCheckedChange={() => toggleAutomationStatus(automation.id, automation.status)}/>
