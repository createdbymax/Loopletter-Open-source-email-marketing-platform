...
export function CampaignEditor({ campaignId }: CampaignEditorProps) {
    ...
    const handleSave = async (htmlContent: string) => {
        if (!campaign || !artist)
            return;
        console.log("Campaign Editor - handleSave called with content length:", htmlContent.length);
        console.log("Campaign Editor - HTML content preview:", htmlContent.substring(0, 200) + "...");
        try {
            ...
            console.log("Campaign Editor - Sending update with complete HTML length:", completeHtml.length);
            const response = await fetch(`/api/campaigns/${campaignId}`, {
                method: "PUT",
    ...
    if (campaign.status === "sent") {
        ...
        <button onClick={() => router.push("/dashboard/campaigns")} className="mt-4 text-blue-600 hover:text-blue-800">
          Back to Campaigns
        </button>
      </div>);
    }
    ...
    const isSpotifyGenerated = templateId === "spotify-generated" ||
        (campaign.template_data as any)?._originalTemplateId ===
            "spotify-generated";
    const hasBeenEdited = campaign.message &&
    ...
    return (<div>
      ...
      {hasBeenEdited && (<div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4 mx-6 mt-4">
      ...
      <MailyEditor initialHtml={initialHtml} templateId={useTemplateData ? templateId : undefined} templateData={useTemplateData ? campaign.template_data : undefined} fanCount={fanCount} subscriptionPlan={getUserSubscriptionPlan(artist)} subject={campaign.subject || ""} previewText={campaign.preview_text || ""} fromName={campaign.from_name || artist.default_from_name || artist.name} artist={artist} onBack={() => router.push("/dashboard/campaigns")} onSave={handleSave} onSend={handleSend}/>

      
      <CampaignSendingModal isOpen={showSendingModal} onClose={handleCloseSendingModal} campaignId={campaignId} fanCount={fanCount} onComplete={handleSendingComplete}/>
    </div>);
}
