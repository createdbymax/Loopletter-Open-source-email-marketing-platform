...
export function CampaignBuilder() {
  ...
  const [currentStep, setCurrentStep] = useState<'template' | 'form' | 'editor'>('template');
  ...
  // Fetch fan count on component mount
  useEffect(() => {
  ...
  const handleSaveDraft = async () => {
    try {
      const campaignData = {
        ...
        message: selectedTemplate ? '' : blocks.map(b => b.content).join('\n'),
  ...
  const getDefaultStyles = (type: Block['type']) => {
    switch (type) {
      case 'heading': return { fontSize: 20, fontWeight: 'bold' };
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Design your email campaign</p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Know when recipients open your emails</p>
                  </div>
                  <Switch
                    checked={campaignSettings.track_opens}
                    onCheckedChange={(checked) => 
                      setCampaignSettings(prev => ({ ...prev, track_opens: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Track which links recipients click</p>
                  </div>
                  <Switch
                    checked={campaignSettings.track_clicks}
                    onCheckedChange={(checked) => 
                      setCampaignSettings(prev => ({ ...prev, track_clicks: checked }))
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Send at optimal times for each recipient</p>
                  </div>
                  <Switch
                    checked={campaignSettings.send_time_optimization}
                    onCheckedChange={(checked) => 
                      setCampaignSettings(prev => ({ ...prev, send_time_optimization: checked }))
                    }
                  />
                </div>
              </div>
    ...
    >
      ...
      {block.type === 'text' && (
        isEditing ? (
          ...
          <div style={blockStyles} className="min-h-[1.5em] whitespace-pre-wrap">
