...
import { ArrowLeft, Send, Eye, Save, Plus, Type, Image as ImageIcon, Link, Palette, Bold, Italic, AlignLeft, AlignCenter, Monitor, Smartphone, } from "lucide-react";
...
export function CampaignBuilder() {
    ...
    const [currentStep, setCurrentStep] = useState<"template" | "form" | "maily-editor">("template");
    ...
    useEffect(() => {
        async function fetchData() {
            if (!user || !isLoaded)
                return;
            try {
                const artistData = await getOrCreateArtistByClerkId(user.id, user.primaryEmailAddress?.emailAddress || "", user.fullName || user.username || "Artist");
    ...
    const handleSendCampaign = async () => {
        ...
        if (currentStep === "maily-editor" &&
            ...
            alert("Please save your email content first by clicking the 'Save Draft' button, then try sending again.");
            return;
        }
        try {
            const campaignData = {
                ...
                from_name: emailData.fromName || artist?.default_from_name || artist?.name,
                from_email: artist?.ses_domain
                    ? `${artist.default_from_email || "noreply"}@${artist.ses_domain}`
                    : null,
                message: emailHtml,
    ...
    if (currentStep === "form") {
        ...
        return (<TemplateForm templateId={selectedTemplate} onBack={() => setCurrentStep("template")} onContinue={handleTemplateFormComplete}/>);
    }
    if (currentStep === "maily-editor") {
        return (<>
        <MailyEditor initialHtml={emailHtml} templateId={selectedTemplate || undefined} templateData={templateData} fanCount={fanCount} subscriptionPlan={artist ? getUserSubscriptionPlan(artist) : "starter"} subject={emailData.subject} previewText={emailData.previewText} fromName={emailData.fromName} artist={artist} onBack={() => setCurrentStep("template")} onSave={(htmlContent: string) => {
        ...
        {campaignToSend && (<CampaignSendingModal isOpen={showSendingModal} onClose={handleCloseSendingModal} campaignId={campaignToSend} fanCount={fanCount} onComplete={handleSendingComplete}/>)}
      </>);
    }
    ...
    return (<div className="min-h-screen bg-gray-50 dark:bg-neutral-900">
      ...
      <div className="bg-white dark:bg-neutral-800 border-b dark:border-neutral-700 px-6 py-4">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={handleSaveDraft} disabled={!isDomainValid}>
            ...
            <Button size="sm" onClick={handleSendCampaign} disabled={!isDomainValid ||
            Boolean(artist && hasReachedEmailSendLimit(artist, fanCount))} title={artist && hasReachedEmailSendLimit(artist, fanCount)
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        {user && (<div className="mb-6">
            {artist && (<EmailLimitWarning artist={artist} currentCount={fanCount}/>)}
          </div>)}
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-3 space-y-6">
            ...
            subject: e.target.value,
                ...
                <div>
                  ...
                  <Input id="fromName" value={emailData.fromName} onChange={(e) => setEmailData((prev) => ({
            ...prev,
            ...
            {!selectedTemplate && (<div className="bg-white dark:bg-neutral-800 rounded-lg border dark:border-neutral-700 p-4">
                <h3 className="font-medium mb-4 text-neutral-900 dark:text-neutral-100">
                  Add Content
                </h3>
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" size="sm" onClick={() => addBlock("text")} className="flex flex-col h-16 gap-1">
                    <Type className="w-4 h-4"/>
                    <span className="text-xs">Text</span>
                  </Button>

                  <Button variant="outline" size="sm" onClick={() => addBlock("heading")} className="flex flex-col h-16 gap-1">
                    <Type className="w-4 h-4 font-bold"/>
                    <span className="text-xs">Heading</span>
                  </Button>

                  <Button variant="outline" size="sm" onClick={() => addBlock("image")} className="flex flex-col h-16 gap-1">
                    <ImageIcon className="w-4 h-4"/>
                    <span className="text-xs">Image</span>
                  </Button>

                  <Button variant="outline" size="sm" onClick={() => addBlock("button")} className="flex flex-col h-16 gap-1">
            ...
            <div className="bg-white dark:bg-neutral-800 rounded-lg border dark:border-neutral-700">
              ...
              <div className="p-6">
                <div className={`mx-auto ${previewMode === "mobile" ? "max-w-sm" : "max-w-2xl"}`}>
                  {selectedTemplate &&
            templateData &&
            Object.keys(templateData).length > 0 ? (<TemplatePreview templateId={selectedTemplate} templateData={templateData as any}/>) : (<div className="bg-white dark:bg-neutral-900 border dark:border-neutral-700 rounded-lg shadow-sm">
                      <div className="p-6 space-y-4">
                        {blocks.map((block) => (<BlockEditor key={block.id} block={block} isSelected={selectedBlockId === block.id} onSelect={() => setSelectedBlockId(block.id)} onUpdate={(updates) => updateBlock(block.id, updates)} onDelete={() => deleteBlock(block.id)}/>))}

                        
                        <Button variant="ghost" className="w-full border-2 border-dashed border-gray-300 dark:border-neutral-600 h-12 text-gray-500 dark:text-neutral-400 hover:border-gray-400 dark:hover:border-neutral-500" onClick={() => addBlock("text")}>
          ...
          <div className="col-span-3">
            {selectedTemplate && templateData ? (<TemplateEditor templateId={selectedTemplate} templateData={templateData} onUpdate={setTemplateData}/>) : (selectedBlockId && (<BlockProperties block={blocks.find((b) => b.id === selectedBlockId)!} onUpdate={(updates) => updateBlock(selectedBlockId, updates)}/>))}
          </div>
        </div>
      </div>
...
function BlockEditor({ block, isSelected, onSelect, onUpdate, onDelete, }: BlockEditorProps) {
    ...
    return (<div className={`relative group cursor-pointer rounded-md transition-all ${isSelected
            ? "ring-2 ring-blue-500 ring-offset-2"
            : "hover:bg-gray-50 dark:hover:bg-neutral-800"}`} onClick={handleClick}>
      
      {isSelected && (<Button variant="destructive" size="sm" className="absolute -top-2 -right-2 w-6 h-6 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => {
                e.stopPropagation();
                onDelete();
            }}>
          ×
      ...
      {block.type === "text" &&
            (isEditing ? (<textarea ref={inputRef} value={block.content} onChange={(e) => onUpdate({ content: e.target.value })} onBlur={handleBlur} onKeyDown={handleKeyDown} style={blockStyles} className="w-full bg-transparent border-none outline-none resize-none" rows={3}/>) : (<div style={blockStyles} className="min-h-[1.5em] whitespace-pre-wrap">
            {block.content || "Click to edit text..."}
          </div>))}

      {block.type === "heading" &&
            (isEditing ? (<input ref={inputRef as unknown as React.RefObject<HTMLInputElement>} value={block.content} onChange={(e) => onUpdate({ content: e.target.value })} onBlur={handleBlur} onKeyDown={handleKeyDown} style={blockStyles} className="w-full bg-transparent border-none outline-none"/>) : (<h2 style={blockStyles} className="min-h-[1.2em]">
            {block.content || "Click to edit heading..."}
          </h2>))}

      {block.type === "image" && (<div className="space-y-2">
          <Input value={block.content} onChange={(e) => onUpdate({ content: e.target.value })} placeholder="Enter image URL..." className="text-sm"/>
          {block.content && (<img src={block.content} alt="" className="max-w-full h-auto rounded" style={blockStyles}/>)}
        </div>)}

      {block.type === "button" && (<div className="space-y-2">
          <Input value={block.content} onChange={(e) => onUpdate({ content: e.target.value })} placeholder="Button text..." className="text-sm"/>
          <Input value={block.href || ""} onChange={(e) => onUpdate({ href: e.target.value })} placeholder="Button link (https://...)..." className="text-sm"/>
          <button style={blockStyles} className="px-6 py-3 rounded-md font-medium">
            {block.content || "Button"}
          </button>
        </div>)}

      {block.type === "divider" && (<hr className="border-gray-300 my-4" style={blockStyles}/>)}
    </div>);
}
...
function BlockProperties({ block, onUpdate }: BlockPropertiesProps) {
    ...
    return (<div className="bg-white rounded-lg border p-4 space-y-4">
      <h3 className="font-medium">Block Properties</h3>

      <div className="space-y-3">
        <div>
          <Label>Font Size</Label>
          <Input type="number" value={block.styles?.fontSize || 16} onChange={(e) => updateStyle("fontSize", parseInt(e.target.value))} min="8" max="72"/>
        </div>

        <div>
          <Label>Text Color</Label>
          <Input type="color" value={block.styles?.color || "#000000"} onChange={(e) => updateStyle("color", e.target.value)}/>
        </div>

        {block.type === "button" && (<div>
            <Label>Background Color</Label>
            <Input type="color" value={block.styles?.backgroundColor || "#2563eb"} onChange={(e) => updateStyle("backgroundColor", e.target.value)}/>
          </div>)}

        <div>
          <Label>Text Alignment</Label>
          <div className="flex gap-1 mt-1">
            {["left", "center", "right"].map((align) => (<Button key={align} variant={block.styles?.textAlign === align ? "default" : "outline"} size="sm" onClick={() => updateStyle("textAlign", align)} className="flex-1">
                {align}
              </Button>))}
          </div>
        </div>
