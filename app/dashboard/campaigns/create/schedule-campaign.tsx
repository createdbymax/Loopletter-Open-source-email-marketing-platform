...
export function ScheduleCampaign({ artist, onSchedule }: ScheduleCampaignProps) {
    ...
    return (<FeatureGate feature="emailScheduling" artist={artist} fallback={<div className="bg-white rounded-lg border p-4 space-y-4">
      ...
      <div className="bg-white rounded-lg border p-4 space-y-4">
        ...
        <div className="space-y-3">
          <div>
            <Label>Date</Label>
            <Input type="date" value={date ? format(date, 'yyyy-MM-dd') : ''} onChange={(e) => setDate(e.target.value ? new Date(e.target.value) : undefined)} className="mt-1"/>
          </div>
