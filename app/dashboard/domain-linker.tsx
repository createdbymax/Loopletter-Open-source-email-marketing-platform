...
export function DomainLinker() {
    ...
    React.useEffect(() => {
        async function fetchArtist() {
            if (!user)
                return;
            const a = await getOrCreateArtistByClerkId(user.id, user.primaryEmailAddress?.emailAddress || user.emailAddresses[0]?.emailAddress || "", user.fullName || user.username || "Artist");
            setArtist(a);
    ...
    return (<TooltipProvider>
      <div className="w-full mx-auto space-y-6">
        ...
        {currentDomain && (<div className="bg-muted/50 border rounded p-4 space-y-3 mb-4">
            ...
            {currentDomain && currentStatus !== "verified" && (<div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                <Loader2 className="w-3 h-3 animate-spin"/>
                Auto-refreshing every 5s
              </div>)}
            {dnsRecords.length > 0 ? (<Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {dnsRecords.map((rec, i) => (<TableRow key={i}>
              ...
              </Table>) : (<div className="text-xs text-muted-foreground">No DNS records to display.</div>)}
        ...
        {step === 0 && (<form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-muted/50 border rounded p-4">
            <div className="flex items-center gap-2">
              <Input placeholder="yourdomain.com" value={domain} onChange={e => setDomain(e.target.value)} disabled={loading} className="flex-1" required/>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Info className="w-4 h-4 text-muted-foreground cursor-pointer"/>
                </TooltipTrigger>
        ...
        {step === 2 && (<div className="bg-green-50 border border-green-200 rounded p-4 flex items-center gap-3">
        ...
        {currentDomain && (<div className="bg-muted/50 border rounded p-4 space-y-4">
