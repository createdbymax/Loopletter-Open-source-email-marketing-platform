...
export function Fans() {
    ...
    return (<section className="mb-8">
      ...
      {pasting && (<div className="border p-3 mb-2 rounded">
          <div className="mb-2">Paste contacts (one per line, comma/tab separated: email[, name[, tags]]):</div>
          <Textarea className="mb-2" rows={6} value={pasteText} onChange={e => setPasteText(e.target.value)} placeholder={`alice@email.com\nbob@email.com, Bob\ncarol@email.com, Carol, friend;VIP`}/>
      ...
      {importing && (<div className="border p-3 mb-2 rounded">
          ...
          <Table className="mb-2">
            ...
            <TableBody>
              {csvPreview.map((row, i) => (<TableRow key={i}>
      ...
      {!loading && fans.length > 0 && (<Table>
          ...
          <TableBody>
            {fans.map(fan => (<TableRow key={fan.id}>
                ...
                <TableCell>
                  <Input value={editing[fan.id] ?? (fan.tags?.join(', ') || '')} onChange={e => handleTagChange(fan, e.target.value)}/>
