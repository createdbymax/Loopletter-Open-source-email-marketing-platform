...
import { Plus, Users, Mail, Download, Upload, MoreHorizontal, UserMinus, Trash2, UserCheck, } from "lucide-react";
...
export function FansManager() {
    ...
    const handleFanAction = async (fanId: string, action: "unsubscribe" | "resubscribe") => {
        ...
        try {
            ...
            setFans((prevFans) => prevFans.map((fan) => fan.id === fanId
                ? {
                    ...fan,
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowAddForm(true)} disabled={artist
      ...
      {artist && (<SubscriberLimitWarning artist={artist} currentCount={fans.length}/>)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg border">
          ...
          <p className="mt-2 text-3xl font-bold text-gray-900">
            ...
            const now = new Date();
            return (fanDate.getMonth() === now.getMonth() &&
      ...
      {showAddForm && (<div className="bg-white rounded-lg border p-6">
          <h3 className="text-lg font-medium mb-4">Add New Fan</h3>
          <form onSubmit={handleAddFan} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="email">Email *</Label>
                <Input id="email" type="email" value={newFan.email} onChange={(e) => setNewFan((prev) => ({ ...prev, email: e.target.value }))} placeholder="fan@example.com" required/>
              </div>
              <div>
                <Label htmlFor="name">Name</Label>
                <Input id="name" value={newFan.name} onChange={(e) => setNewFan((prev) => ({ ...prev, name: e.target.value }))} placeholder="Fan Name"/>
              </div>
            </div>
            <div className="flex gap-2">
      ...
      <div className="bg-white rounded-lg border">
        {fans.length === 0 ? (<div className="text-center py-12">
            ...
            <TableBody>
              {fans.map((fan) => (<TableRow key={fan.id}>
                  ...
                  <TableCell>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${fan.status === "subscribed"
                  ...
                  <TableCell>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="h-8 w-8 p-0" disabled={actionLoading === fan.id}>
                          <MoreHorizontal className="h-4 w-4"/>
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        {fan.status === "subscribed" ? (<DropdownMenuItem onClick={() => handleFanAction(fan.id, "unsubscribe")} disabled={actionLoading === fan.id}>
                            <UserMinus className="mr-2 h-4 w-4"/>
                            Unsubscribe
                          </DropdownMenuItem>) : (<DropdownMenuItem onClick={() => handleFanAction(fan.id, "resubscribe")} disabled={actionLoading === fan.id}>
                            <UserCheck className="mr-2 h-4 w-4"/>
                            Resubscribe
                          </DropdownMenuItem>)}
                        <DropdownMenuItem onClick={() => setDeleteDialog({ open: true, fan })} disabled={actionLoading === fan.id} className="text-red-600">
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ open, fan: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteDialog.fan && handleDeleteFan(deleteDialog.fan)} className="bg-red-600 hover:bg-red-700" disabled={actionLoading === deleteDialog.fan?.id}>
