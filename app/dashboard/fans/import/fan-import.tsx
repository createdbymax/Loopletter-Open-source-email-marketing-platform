...
import { Upload, FileSpreadsheet, CheckCircle, XCircle, AlertTriangle, Download, RefreshCw, Eye, ArrowRight } from 'lucide-react';
...
export function FanImport() {
    ...
    const handleCSVImport = async () => {
        if (!file || !columnMapping.email || loading)
            return;
        setLoading(true);
        setProgress(0);
        setResult(null);
        ...
        <TabsContent value="csv" className="space-y-4">
          {!showPreview ? (<Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className={`border-2 border-dashed rounded-lg p-8 text-center ${file ? 'border-green-300 bg-green-50' : 'border-gray-300 hover:border-gray-400'}`} onDragOver={handleDragOver} onDrop={handleDrop}>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="overflow-x-auto max-w-full">
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            {csvPreview?.headers.map((header, index) => (<th key={index} className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[100px] max-w-[200px]">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {csvPreview?.rows.map((row, rowIndex) => (<tr key={rowIndex}>
                              {row.map((cell, cellIndex) => (<td key={cellIndex} className="px-3 py-2 text-sm text-gray-900 min-w-[100px] max-w-[200px]">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Select value={columnMapping.email} onValueChange={(value) => setColumnMapping(prev => ({ ...prev, email: value }))}>
                          <SelectTrigger>
                            <SelectValue placeholder="Select email column"/>
                          </SelectTrigger>
                          <SelectContent>
                            {csvPreview?.headers.map((header) => (<SelectItem key={header} value={header}>
                                {header}
                              </SelectItem>))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select value={columnMapping.name || "none"} onValueChange={(value) => setColumnMapping(prev => ({ ...prev, name: value === "none" ? "" : value }))}>
                          <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="tags" placeholder="Enter comma-separated tags (e.g. newsletter, website)" value={tags} onChange={(e) => setTags(e.target.value)}/>
                  ...
                  <div className="flex items-center space-x-2">
                    <Checkbox id="skip-duplicates" checked={skipDuplicates} onCheckedChange={(checked) => setSkipDuplicates(checked as boolean)}/>
                ...
                <CardFooter className="flex justify-between">
                  ...
                  <Button onClick={handleCSVImport} disabled={!columnMapping.email || loading} className="min-w-[140px]">
                    {loading ? (<>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea id="emails" placeholder="Enter one email address per line" rows={10} value={manualEmails} onChange={(e) => setManualEmails(e.target.value)}/>
                <p className="text-xs text-gray-500">
              ...
              <div className="space-y-2">
                ...
                <Input id="manual-tags" placeholder="Enter comma-separated tags (e.g. newsletter, website)" value={tags} onChange={(e) => setTags(e.target.value)}/>
              </div>

              
              <div className="flex items-center space-x-2">
                <Checkbox id="manual-skip-duplicates" checked={skipDuplicates} onCheckedChange={(checked) => setSkipDuplicates(checked as boolean)}/>
            ...
            <CardFooter className="flex justify-between">
              ...
              <Button onClick={handleManualImport} disabled={!manualEmails.trim() || loading} className="min-w-[120px]">
                {loading ? (<>
                    <RefreshCw className="h-4 w-4 mr-2 animate-spin"/>
                    Importing...
                  </>) : ('Import Fans')}
      ...
      {loading && (<Card>
          <CardContent className="pt-6">
            <div className="space-y-3">
              ...
              {jobStatus && (<div className="text-xs text-center text-gray-500 space-y-1">
      ...
      {result && (<Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-center gap-8">
              ...
              {result.failed !== undefined && (<div className="text-center">
                  <p className="text-3xl font-bold text-red-600">{result.failed}</p>
                  <p className="text-sm text-gray-600">Failed to Import</p>
                </div>)}
            </div>

            {result.message && (<div className="text-center">
            ...
            {result.errors && result.errors.length > 0 && (<div className="space-y-2">
                ...
                <div className="max-h-60 overflow-y-auto border rounded-lg">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {result.errors.map((error, index) => (<tr key={index}>
