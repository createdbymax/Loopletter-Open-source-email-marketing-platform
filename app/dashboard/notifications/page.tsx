...
import { Bell, Check, CheckCheck, Clock, AlertCircle, FileText, Trash2 } from 'lucide-react';
...
export default function NotificationsPage() {
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      ...
      {loading ? (<div className="text-center py-12">
        ...
        </div>) : notifications.length === 0 ? (<Card>
          ...
          {notifications.map((notification) => (<Card key={notification.id} className={`transition-colors ${!notification.read ? 'bg-blue-50 border-blue-200' : ''}`}>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        {!notification.read && (<Badge variant="secondary" className="bg-blue-100 text-blue-800">
                            New
                          </Badge>)}
                    ...
                    {notification.data && (<div className="bg-gray-50 rounded-lg p-3 mb-3">
                        {notification.type === 'import_completed' && (<div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            <div>
                              <span className="font-medium text-green-600">Imported:</span>
                              <p className="text-gray-900">{notification.data.imported}</p>
                            </div>
                            {notification.data.failed > 0 && (<div>
                                <span className="font-medium text-red-600">Failed:</span>
                                <p className="text-gray-900">{notification.data.failed}</p>
                              </div>)}
                            {notification.data.skipped > 0 && (<div>
                        ...
                        {notification.type === 'import_failed' && (<div className="text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {notification.type === 'import_completed' && (<Button variant="outline" size="sm" asChild>
                          <a href="/dashboard/fans">View Fans</a>
                        </Button>)}
                      
                      {notification.type === 'import_failed' && (<Button variant="outline" size="sm" asChild>
