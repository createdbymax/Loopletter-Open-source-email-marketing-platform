...
import { Target, Plus, Trash2, Edit, Users, Tag, Mail, Calendar, MousePointer, Lock } from 'lucide-react';
...
export function SegmentManager({ artist }: SegmentManagerProps) {
    ...
    return (<FeatureGate feature="segmentation" artist={artist} fallback={<div className="space-y-6">
            ...
            <DialogContent className="sm:max-w-[600px]">
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input id="segment-name" value={newSegment.name} onChange={(e) => setNewSegment({ ...newSegment, name: e.target.value })} placeholder="e.g., Active Subscribers, Recent Fans"/>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="segment-description">Description (Optional)</Label>
                  <Input id="segment-description" value={newSegment.description} onChange={(e) => setNewSegment({ ...newSegment, description: e.target.value })} placeholder="What is this segment for?"/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-3 mt-3">
                    {newSegment.conditions.map((condition, index) => (<div key={index} className="flex items-center gap-2">
                        {index > 0 && (<Select value={condition.logic} onValueChange={(value) => updateCondition(index, 'logic', value)}>
                        ...
                        <Input value={typeof condition.value === 'boolean' ? String(condition.value) : condition.value} onChange={(e) => {
                ...
                updateCondition(index, 'value', newValue);
                        ...
                        <Button variant="ghost" size="icon" onClick={() => removeCondition(index)} disabled={newSegment.conditions.length <= 1}>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleCreateSegment} disabled={isCreating || !newSegment.name}>
        ...
        {loading ? (<div className="flex justify-center py-12">
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {segments.map((segment) => (<TableRow key={segment.id}>
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-2">
                          <Target className="h-4 w-4 text-blue-600"/>
                          {segment.name}
                        </div>
                        {segment.description && (<div className="text-xs text-gray-500 mt-1">
                            {segment.description}
                          </div>)}
                      </TableCell>
                      <TableCell>
                        <div className="space-y-1">
                          {segment.conditions.map((condition, i) => (<div key={i} className="flex items-center gap-1 text-sm">
                              {i > 0 && (<span className="text-xs font-medium bg-gray-100 px-1.5 py-0.5 rounded">
