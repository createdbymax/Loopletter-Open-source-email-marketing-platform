...
import { Plus, ShoppingCart, Music, Instagram, Twitter, Zap, CreditCard, Globe, Settings, CheckCircle, AlertCircle, ExternalLink, } from "lucide-react";
...
function IntegrationCard({ integration, connectedIntegration, onConnect, onDisconnect, }: {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isConnected && (<div className="flex items-center gap-1">
                {hasError ? (<AlertCircle className="w-4 h-4 text-red-500"/>) : (<CheckCircle className="w-4 h-4 text-green-500"/>)}
                <Badge variant={hasError ? "destructive" : "default"} className={hasError ? "" : "bg-green-100 text-green-800"}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <ul className="text-sm text-gray-600 space-y-1">
            {integration.features.map((feature, index) => (<li key={index} className="flex items-center gap-2">
        ...
        {connectedIntegration && (<div className="text-xs text-gray-500">
            Last synced:{" "}
        ...
        <div className="flex gap-2">
          <Button size="sm" disabled onClick={() => alert("Integrations are coming soon! We're working on OAuth flows and proper API connections for all supported platforms.")}>
...
function WebhookManager({ artistId }: {
    artistId: string;
}) {
    return (<div className="space-y-6">
      <Card>
        ...
        ].map((event) => (<div key={event} className="flex items-center gap-2">
...
export function IntegrationsManager() {
    ...
    const handleConnect = async (integrationType: string) => {
        alert("Integration connections are coming soon! We're working on OAuth flows and proper API integrations for all supported platforms.");
    };
    ...
    return (<div className="space-y-6">
      ...
      <Tabs defaultValue="integrations">
        ...
        <TabsContent value="integrations" className="space-y-6">
          {Object.entries(groupedIntegrations).map(([category, categoryIntegrations]) => (<div key={category} className="space-y-4">
                <h2 className="text-xl font-semibold capitalize">{category}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {categoryIntegrations.map((integration) => (<IntegrationCard key={integration.type} integration={integration} connectedIntegration={getConnectedIntegration(integration.type)} onConnect={() => handleConnect(integration.type)} onDisconnect={() => {
