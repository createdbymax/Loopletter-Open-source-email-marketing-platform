...
export default function SubscriptionSettings({ artist }: SubscriptionSettingsProps) {
  ...
  const handlePlanChange = async (planId: string, billingInterval: 'month' | 'year' = 'month') => {
    if (planId === currentPlan || (planId === 'starter' && currentPlan !== 'starter')) {
  ...
  const handleManageSubscription = async () => {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        alert(`Failed to access subscription management: ${error.message || 'Unknown error'}. Check the console for more details.`);
      } else {
    ...
    <div className="space-y-8">
      ...
      {canceled && (
        <Alert className="bg-amber-50 border-amber-200">
          <AlertDescription className="text-amber-800">
            The subscription change was canceled. Your current plan remains active.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Tabs defaultValue="plans">
        ...
        <TabsContent value="plans" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Current Plan: {plans.find(p => p.id === currentPlan)?.name || 'Starter'}</CardTitle>
              <CardDescription>
                {currentPlan === 'starter' 
                ...
                <div>
                  <p className="text-sm text-gray-600">
                    {currentPlan === 'starter' 
                      ...
                      : 'Your subscription includes all features of the ' + (currentPlan === 'independent' ? 'Independent' : 'Label/Agency') + ' plan.'}
                  </p>
                  
                  {artist.subscription?.current_period_end && (
                    <p className="text-sm text-gray-600 mt-4 flex items-center">
                      <Calendar className="h-4 w-4 mr-2 text-gray-500" />
                      Next billing date: {formatDate(artist.subscription.current_period_end)}
                    </p>
                  )}
                  ...
                  {artist.subscription?.cancel_at_period_end && (
                    <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-md">
                      <p className="text-sm text-amber-800">
                        Your subscription will end on {formatDate(artist.subscription.current_period_end)}. 
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      {plans.map((plan) => (
                        ...
                        >
                          {plan.name}
                          <div className="font-semibold text-sm mt-1">{plan.price}/mo</div>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {keyFeatures.map((feature) => (
                      <tr key={feature} className={`hover:bg-gray-50 ${highlightedFeature === feature ? 'bg-blue-50' : ''}`}>
                    ...
                    {currentPlan === plan.id ? (
                      ...
                      <Button
                        ...
                        className={`mt-6 w-full ${
                          plan.id === 'starter' || currentPlan === 'label' && plan.id === 'independent'
                            ? 'bg-gray-100 text-gray-800 hover:bg-gray-200 border border-gray-300'
                            : ''
                        }`}
                        variant={plan.id === 'starter' || currentPlan === 'label' && plan.id === 'independent' ? 'outline' : 'default'}
                      >
                        {isLoading && loadingPlan === plan.id ? (
                          ...
                          'Downgrade'
                        ) : currentPlan === 'independent' && plan.id === 'label' ? (
                          <>Upgrade <ArrowRight className="ml-1 h-4 w-4" /></>
          ...
          <Card>
            ...
            <CardContent>
              {currentPlan === 'starter' ? (
                ...
                <p className="text-gray-600">
                  No invoices found. Your billing history will appear here once you&apos;ve been billed.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
