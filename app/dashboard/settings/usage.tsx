...
export default function Usage({ artist }: UsageProps) {
  ...
  return (
    <div className="space-y-6">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full ${
                      subscriberPercentage >= 90 ? 'bg-red-500' : 
                      subscriberPercentage >= 75 ? 'bg-amber-500' : 
                      'bg-blue-600'
                    }`}
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>0</span>
                  <span>{maxSubscribers === 'unlimited' ? 'Unlimited' : maxSubscribers.toLocaleString()}</span>
                </div>
                
                <div className="pt-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-gray-600">Active Subscribers</span>
                    <span className="font-medium">{stats.activeFans.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-gray-600">Unsubscribed</span>
                    <span className="font-medium">{(stats.totalFans - stats.activeFans).toLocaleString()}</span>
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Monthly Email Sends</span>
                  <span className="font-medium">
                    {stats.monthlyEmailsSent.toLocaleString()} / {maxEmailSends === 'unlimited' ? 'Unlimited' : maxEmailSends.toLocaleString()}
                  </span>
                </div>
                ...
                {maxEmailSends !== 'unlimited' ? (
                  <>
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${
                          emailPercentage >= 90 ? 'bg-red-500' : 
                          emailPercentage >= 75 ? 'bg-amber-500' : 
                          'bg-blue-600'
                        }`}
                ...
                <div className="pt-2">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="font-medium">{stats.totalCampaigns.toLocaleString()}</span>
                  </div>
                </div>
                
                {maxEmailSends !== 'unlimited' && emailPercentage >= 75 && (
                  <div className="pt-2">
        ...
        <CardContent>
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Your {plan === 'starter' ? 'Starter' : plan === 'independent' ? 'Independent' : 'Label/Agency'} plan includes:
            </p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
              {Object.entries(getUserLimits(artist)).map(([key, value]) => (
                <div key={key} className="flex items-center justify-between py-1 border-b border-gray-100">
                  <span className="text-sm">
                    {key === 'maxSubscribers' ? 'Maximum Subscribers' : 'Monthly Email Sends'}
                  </span>
                  <span className="font-medium">
              ...
              {/* List key features for the plan */}
              {['emailScheduling', 'advancedAnalytics', 'segmentation', 'automations', 
                'removeLoopLetterBranding', 'customSignupDomain', 'customEmailDesign',
                'premiumSupport', 'prioritySupport', 'multiArtistManagement', 'multiUserAccess'].map(feature => {
                const hasFeature = plan === 'label' || 
                  (plan === 'independent' && !['prioritySupport', 'multiArtistManagement', 'multiUserAccess'].includes(feature));
                
                if (plan === 'starter' && !['manualCampaignSending', 'hostedSignupPage', 'autoResponseWelcomeEmail', 'basicAnalytics', 'communitySupport'].includes(feature)) {
                  return null;
                }
                
                return (
                  <div key={feature} className="flex items-center justify-between py-1 border-b border-gray-100">
                    <span className="text-sm">{FEATURE_NAMES[feature as keyof FeatureAccess]}</span>
                    {hasFeature ? (
                      <CheckCircle className="h-4 w-4 text-green-600" />
                    ) : (
                      <Link href={`/dashboard/settings?tab=subscription&feature=${feature}`}>
                        <span className="text-xs text-blue-600 hover:underline">Upgrade</span>
                      </Link>
                    )}
                  </div>
                );
      ...
      {/* Billing Period */}
      {plan !== 'starter' && artist.subscription?.current_period_end && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {artist.subscription.cancel_at_period_end && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-medium text-amber-800">Subscription Cancellation</h4>
                  </div>
                  <p className="text-xs text-amber-700 mt-1">
                    Your subscription has been canceled and will end on {new Date(artist.subscription.current_period_end).toLocaleDateString()}.
