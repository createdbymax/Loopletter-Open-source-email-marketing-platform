...
import { Link as LinkIcon, Edit3, Check, X, AlertCircle, Copy, CheckCircle, RefreshCw } from 'lucide-react';
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
            ...
            <button onClick={() => copyToClipboard(currentUrl)} className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded-lg transition-colors" title="Copy URL">
        ...
        <div>
          ...
          {!isEditing ? (<div className="flex items-center gap-3 p-4 border border-gray-300 rounded-lg">
              ...
              <button onClick={() => setIsEditing(true)} className="ml-auto p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors" title="Edit slug">
              ...
              <div className="flex items-center gap-2 p-4 border border-gray-300 rounded-lg">
                ...
                <input type="text" value={newSlug} onChange={(e) => handleSlugChange(e.target.value)} className="flex-1 bg-transparent outline-none font-medium text-gray-900" placeholder="your-slug" maxLength={50}/>
              ...
              {!isChecking && isAvailable !== null && (<div className={`flex items-center gap-2 text-sm ${isAvailable ? 'text-green-600' : 'text-red-600'}`}>
                  {isAvailable ? (<>
              ...
              {newSlug && isAvailable && (<div className="p-3 bg-green-50 rounded-lg">
              ...
              <div className="flex gap-3">
                <button onClick={handleSave} disabled={!newSlug || !isAvailable || isSaving} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  {isSaving ? (<>
                ...
                <button onClick={handleCancel} className="flex items-center gap-2 px-4 py-2 text-gray-600 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors">
                  <X className="w-4 h-4"/>
                  Cancel
                </button>
              </div>
        ...
        {!isEditing && (<div>
            ...
            <div className="flex flex-wrap gap-2">
              {generateSuggestions().map((suggestion, index) => (<button key={index} onClick={() => {
