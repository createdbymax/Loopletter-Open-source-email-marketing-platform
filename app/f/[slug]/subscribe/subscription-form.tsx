...
export default function SubscriptionForm({ artist }: SubscriptionFormProps) {
    ...
    const mergedSettings = {
        ...
        benefits: {
            ...
            custom_benefits: settings?.benefits?.custom_benefits ||
    ...
    if (isSuccess) {
        return (<div className="max-w-md w-full text-center relative z-10">
        ...
        <div className="space-y-4">
          {mergedSettings.benefits.custom_benefits.slice(0, 3).map((benefit, index) => (<div key={index} className="flex items-center justify-center gap-4 text-gray-300">
    ...
    return (<div className="max-w-lg w-full">
      ...
      <div className="text-center mb-12">
        ...
        {mergedSettings.header.show_social_links &&
            artist.settings?.social_links && (<div className="flex justify-center gap-6 mt-8">
              {artist.settings.social_links.spotify && (<a href={artist.settings.social_links.spotify} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400 transition-colors text-sm">
              ...
              {artist.settings.social_links.instagram && (<a href={`https://instagram.com/${artist.settings.social_links.instagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400 transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              {artist.settings.social_links.website && (<a href={artist.settings.social_links.website} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400 transition-colors text-sm">
                  🌐 Website
                </a>)}
            </div>)}
      </div>
      ...
      <div className="bg-gray-900/50 border-gray-800/50 backdrop-blur-xl shadow-2xl rounded-xl">
        <div className="p-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            {mergedSettings.form.show_name_field && (<div className="space-y-3">
                <label htmlFor="name" className="text-sm font-medium text-gray-300">
                  First Name
                </label>
                <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={mergedSettings.form.placeholder_name} className="h-14 w-full bg-gray-800/50 border-gray-700/50 text-white placeholder:text-gray-500 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/25 rounded-xl px-4 outline-none transition-all"/>
              </div>)}
            ...
            <div className="space-y-3">
              ...
              <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={mergedSettings.form.placeholder_email} required className="h-14 w-full bg-gray-800/50 border-gray-700/50 text-white placeholder:text-gray-500 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/25 rounded-xl px-4 outline-none transition-all border"/>
            </div>

            {error && (<div className="bg-red-900/50 border border-red-700/50 rounded-xl p-4">
                <p className="text-red-300 text-sm">{error}</p>
              </div>)}

            <button type="submit" disabled={isLoading || !email} className="w-full h-14 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white font-medium rounded-xl transition-all duration-300 shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 disabled:opacity-50 disabled:cursor-not-allowed">
              {isLoading ? (<div className="flex items-center gap-3 justify-center">
                  <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>
                  <span>Connecting...</span>
                </div>) : (<div className="flex items-center gap-3 justify-center">
          ...
          {mergedSettings.benefits.show_benefits && (<div className="mt-8 pt-8 border-t border-gray-800/50">
              <div className="grid gap-6">
                {mergedSettings.benefits.custom_benefits.slice(0, 3).map((benefit, index) => (<div key={index} className="flex items-start gap-4">
