...
function AcceptInvitationContent() {
    ...
    useEffect(() => {
        async function verifyInvitation() {
            ...
            try {
                const response = await fetch(`/api/team/verify-invitation?token=${token}`);
                if (!response.ok) {
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {!isLoaded ? (<div className="flex justify-center">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div>
              </div>) : isSignedIn ? (<div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="border rounded-lg overflow-hidden">
                  <SignIn redirectUrl={`/team/accept-invitation?token=${token}`}/>
                </div>
              </div>)}
          </div>
        </CardContent>

        {isSignedIn && (<CardFooter className="flex justify-between">
            <Button variant="outline" onClick={() => router.push("/")}>
              Cancel
            </Button>
            <Button onClick={handleAcceptInvitation} disabled={acceptingInvitation}>
