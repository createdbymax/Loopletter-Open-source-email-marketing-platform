...
import { BarChart3, TrendingUp, TrendingDown, Users, Mail, Shield, AlertTriangle, RefreshCw, Calendar, Activity } from 'lucide-react';
    ...
    if (error) {
        ...
        <Alert>
          ...
          <AlertDescription>
            ...
            <Button variant="outline" size="sm" onClick={fetchAnalytics} className="ml-2">
              Retry
            </Button>
          </AlertDescription>
        </Alert>
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={data?.reputation_distribution || []} cx="50%" cy="50%" labelLine={false} label={({ name, percentage }) => `${name}: ${percentage}%`} outerRadius={80} fill="#8884d8" dataKey="count">
                    {(data?.reputation_distribution || []).map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {data?.spam_prevention.top_risk_factors?.slice(0, 5).map((factor, index) => (<div key={index} className="flex justify-between text-sm">
