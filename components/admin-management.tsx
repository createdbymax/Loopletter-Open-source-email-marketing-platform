...
import { Shield, UserPlus, UserMinus, RefreshCw, AlertTriangle, CheckCircle } from 'lucide-react';
...
export function AdminManagement() {
    ...
    return (<div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input id="new-admin-email" type="email" placeholder="Enter email address..." value={newAdminEmail} onChange={(e) => setNewAdminEmail(e.target.value)} disabled={processing}/>
              <Button onClick={handlePromoteUser} disabled={processing || !newAdminEmail.trim()} className="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent>
          {admins.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
              No super admins found
            </div>) : (<div className="space-y-3">
              {admins.map((admin) => (<div key={admin.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <Shield className="h-4 w-4 text-blue-500"/>
                    <div>
                      <div className="font-medium">{admin.email}</div>
                      {admin.promotedAt && (<div className="text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleDemoteUser(admin.email)} disabled={processing} className="text-red-600 hover:text-red-700">
