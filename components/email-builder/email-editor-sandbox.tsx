...
interface EmailEditorSandboxProps {
  ...
  onSave?: (data: { title: string; previewText: string; content: string }) => Promise<void>;
  onUpdate?: (data: { title: string; previewText: string; content: string }) => Promise<void>;
  onSendTest?: (data: { subject: string; previewText: string; from: string; to: string; replyTo: string; content: string }) => Promise<void>;
  fanCount?: number;
...
export function EmailEditorSandbox(props: EmailEditorSandboxProps) {
  const { template, showSaveButton = true, autofocus, onSave, onUpdate, onSendTest, fanCount = 0, setEditor: setParentEditor, subscriptionPlan = 'starter' } = props;
