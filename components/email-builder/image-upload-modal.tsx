      ...
      <DialogContent className="sm:max-w-[500px] mx-4 sm:mx-0 max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="upload" className="w-full">
          ...
          <TabsContent value="upload" className="space-y-4">
            <div className={`border-2 border-dashed rounded-lg p-6 text-center transition-colors ${dragOver
            ...
            : 'border-gray-300 hover:border-gray-400'}`} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave}>
              <div className="space-y-3">
                <div className="mx-auto w-12 h-12 text-gray-400">
                  {uploading ? (<Loader2 className="w-12 h-12 animate-spin"/>) : uploadResult ? (<CheckCircle className="w-12 h-12 text-green-500"/>) : (<ImageIcon className="w-12 h-12"/>)}
                </div>
                <div>
            ...
            {uploadResult && (<div className="space-y-3">
                <div className="flex items-center gap-3 p-3 border rounded-lg bg-green-50">
                  <img src={uploadResult.url} alt="Uploaded image" className="h-16 w-16 object-cover border rounded"/>
        ...
        {imageUrl && (<div className="border rounded-lg p-3">
            <p className="text-sm font-medium mb-2">Preview:</p>
            <img src={imageUrl} alt={altText || 'Preview'} className="max-w-full h-auto max-h-32 object-contain border rounded" onError={() => toast.error('Failed to load image. Please check the URL.')}/>
          </div>)}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleInsertImage} disabled={!imageUrl || uploading}>
            Insert Image
          </Button>
        </DialogFooter>
      </DialogContent>
