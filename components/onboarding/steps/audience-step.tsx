...
export function AudienceStep({ artist, onNext, onStepComplete }: AudienceStepProps) {
  ...
  const updateManualFan = (index: number, field: 'email' | 'name', value: string) => {
    setManualFans(prev => prev.map((fan, i) => 
      i === index ? { ...fan, [field]: value } : fan
    ));
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="relative mb-8">
          ...
          <TabsList className="relative grid w-full grid-cols-3 bg-white/80 backdrop-blur-sm border border-white/20 shadow-sm rounded-2xl p-2">
            <TabsTrigger 
              value="single" 
              className="rounded-xl data-[state=active]:bg-gradient-to-r data-[state=active]:from-green-500 data-[state=active]:to-emerald-500 data-[state=active]:text-white data-[state=active]:shadow-lg transition-all duration-300"
            >
              Add One Fan
            </TabsTrigger>
            <TabsTrigger 
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <Input
                    id="single-email"
                    type="email"
                    value={singleFan.email}
                    onChange={(e) => setSingleFan(prev => ({ ...prev, email: e.target.value }))}
                    placeholder="fan@example.com"
                    className="h-12 bg-white/80 border-gray-200 focus:border-blue-300 focus:ring-blue-200 rounded-xl"
                  />
                </div>
                <div>
                  ...
                  <Input
                    id="single-name"
                    value={singleFan.name}
                    onChange={(e) => setSingleFan(prev => ({ ...prev, name: e.target.value }))}
            ...
            <div className="relative bg-white/80 backdrop-blur-sm p-8 rounded-2xl border border-white/20 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={addManualFan}
                  className="bg-white/80 hover:bg-white border-purple-200 hover:border-purple-300 text-purple-700 rounded-xl"
                >
              ...
              <div className="space-y-4">
                {manualFans.map((fan, index) => (
                  <div key={index} className="flex items-center space-x-4 p-4 bg-white/60 rounded-xl border border-white/40">
                    <div className="flex-1">
                      <Input
                        type="email"
                        value={fan.email}
                        onChange={(e) => updateManualFan(index, 'email', e.target.value)}
                        placeholder="Email address"
                        className="h-12 bg-white/80 border-gray-200 focus:border-purple-300 focus:ring-purple-200 rounded-xl"
                      />
                    </div>
                    <div className="flex-1">
                      <Input
                        value={fan.name}
                        onChange={(e) => updateManualFan(index, 'name', e.target.value)}
                ...
                <div className="border rounded-lg p-4">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {csvPreview.slice(1, 6).map((row: string[], index: number) => (
        ...
        <div className="flex justify-between items-center pt-8">
          ...
          <Button 
            ...
            disabled={loading}
            className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-8 py-3 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-50 disabled:transform-none"
          >
            {loading ? (
              <>
