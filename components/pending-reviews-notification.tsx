...
import { AlertTriangle, Clock, Eye, ChevronRight, X } from 'lucide-react';
import Link from 'next/link';
interface PendingReviewsNotificationProps {
    onDismiss?: () => void;
    showDismiss?: boolean;
}
export function PendingReviewsNotification({ onDismiss, showDismiss = true }: PendingReviewsNotificationProps) {
      ...
      <AlertDescription>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {showDismiss && (<Button size="sm" variant="ghost" onClick={handleDismiss} className="text-yellow-600 hover:text-yellow-800 hover:bg-yellow-100">
                <X className="h-4 w-4"/>
              </Button>)}
          </div>
        </div>
