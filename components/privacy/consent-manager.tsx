...
export function ConsentManager({ 
  ...
  const loadConsentHistory = async () => {
    ...
    try {
      ...
      // Set current consent states based on latest consents
  ...
  const handleConsentChange = async (
    consentType: 'email_marketing' | 'analytics' | 'data_processing',
    granted: boolean
  ) => {
    if (!fanId && !email) {
    ...
    return (
      <div className="space-y-4">
        <div className="space-y-3">
          {Object.entries(consentStates).map(([type, granted]) => (
            <div key={type} className="flex items-center justify-between p-3 border rounded-lg">
              ...
              <Switch
                checked={granted}
                onCheckedChange={(checked) => handleConsentChange(type as any, checked)}
                disabled={saving}
              />
            </div>
          ))}
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                checked={consentStates.email_marketing}
                onCheckedChange={(checked) => handleConsentChange('email_marketing', checked)}
                disabled={saving}
              />
