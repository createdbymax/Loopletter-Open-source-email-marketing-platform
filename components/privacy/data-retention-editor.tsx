...
export function DataRetentionEditor({ 
  open, 
  onOpenChange, 
  artistId,
  onPoliciesUpdated 
}: DataRetentionEditorProps) {
  ...
  const loadCurrentPolicies = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Update form with current policies
  ...
  const handleSave = async () => {
    try {
      ...
      // Save each policy type
      const savePromises = Object.entries(policies).map(([dataType, policy]) =>
        fetch('/api/privacy/retention-policies', {
          method: 'PUT',
          headers: {
  ...
  const handlePolicyChange = (
    dataType: keyof RetentionPolicyForm,
    field: keyof RetentionPolicyForm['fan_data'],
    value: any
  ) => {
    setPolicies(prev => ({
      ...prev,
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {Object.entries(policies).map(([dataType, policy]) => (
              <Card key={dataType}>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) => handlePolicyChange(
                          dataType as keyof RetentionPolicyForm,
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={policy.deletion_method}
                      onValueChange={(value) => handlePolicyChange(
                        dataType as keyof RetentionPolicyForm,
                        'deletion_method',
                        value
                      )}
