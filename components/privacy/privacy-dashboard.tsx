    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {complianceStatus?.data_retention_configured ? (
      ...
      {/* Setup Instructions */}
      {complianceStatus && complianceStatus.issues.includes('Privacy compliance tables not set up yet') && (
        <PrivacySetupInstructions />
      )}

      {/* Issues and Recommendations */}
      {complianceStatus && (complianceStatus.issues.length > 0 || complianceStatus.recommendations.length > 0) && 
       !complianceStatus.issues.includes('Privacy compliance tables not set up yet') && (
      ...
      {/* Only show main content if setup is complete */}
      {complianceStatus && !complianceStatus.issues.includes('Privacy compliance tables not set up yet') && (
        ...
        <TabsContent value="retention" className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button 
              variant="outline" 
              size="sm"
              onClick={() => setRetentionEditorOpen(true)}
            >
              <Settings className="h-4 w-4 mr-2" />
              Configure Policies
            </Button>
      ...
      {/* Privacy Settings Editor Dialog */}
      <PrivacySettingsEditor
        open={settingsEditorOpen}
        onOpenChange={setSettingsEditorOpen}
        artistId={artistId}
        currentSettings={privacySettings}
        onSettingsUpdated={(updatedSettings) => {
          setPrivacySettings(updatedSettings);
          // Reload compliance status after settings update
          loadPrivacyData();
        }}
      />

      {/* Data Retention Editor Dialog */}
      <DataRetentionEditor
        open={retentionEditorOpen}
        onOpenChange={setRetentionEditorOpen}
        artistId={artistId}
        onPoliciesUpdated={() => {
          // Reload compliance status after policies update
          loadPrivacyData();
        }}
      />
    </div>
