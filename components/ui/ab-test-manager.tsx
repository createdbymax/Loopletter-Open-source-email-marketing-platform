...
export function ABTestManager({ artist, campaign, onSave }: ABTestManagerProps) {
  ...
  const handleUpdateSplit = (index: number, value: number) => {
    ...
    // Distribute remaining percentage among other variants
    ...
    const sum = newSplit.reduce((a, b) => a + b, 0);
    if (sum !== 100) {
      // Adjust the last non-edited variant
      for (let i = 0; i < newSplit.length; i++) {
        if (i !== index) {
          newSplit[i] += (100 - sum);
          break;
                  ...
                  <div key={index} className="space-y-2 p-4 border rounded-md">
                    ...
                    {testType === 'subject' && (
                      <Input
                        ...
                        placeholder="Enter subject line variant"
                      />
                    )}
                    ...
                    {testType === 'content' && (
                      <textarea
                        value={variant.content}
                        onChange={(e) => handleUpdateVariant(index, 'content', e.target.value)}
                        placeholder="Enter email content variant"
                    ...
                    {testType === 'from_name' && (
                      <Input
                        value={variant.content}
                        onChange={(e) => handleUpdateVariant(index, 'content', e.target.value)}
                        placeholder="Enter from name variant"
                      />
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => handleUpdateSplit(index, parseInt(e.target.value))}
                          className="flex-1"
                        />
              ...
              <TabsContent value="settings" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setWinnerCriteria(value as any)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
