  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center flex-col bg-gray-50/80 rounded-md">
        ...
        <Link
          href={getUpgradeUrlForFeature(feature)}
...
// Component for displaying a feature lock icon with tooltip
export function FeatureLockIcon({ feature }: { feature: keyof FeatureAccess }) {
  return (
    <div className="inline-flex items-center" title={`Upgrade to access ${FEATURE_NAMES[feature]}`}>
      <Lock className="h-3.5 w-3.5 text-gray-400" />
    </div>
  );
}

// Component for displaying a feature badge (locked/unlocked)
export function FeatureBadge({ feature, artist }: { feature: keyof FeatureAccess; artist: Artist }) {
  const hasAccess = canAccessFeature(artist, feature);
...
// Component for upgrade button
export function UpgradeButton({ feature }: { feature: keyof FeatureAccess }) {
  return (
    <Link
      href={getUpgradeUrlForFeature(feature)}
