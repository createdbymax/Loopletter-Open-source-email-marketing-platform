...
export function LimitWarning({ artist, type, currentCount, className = "", showButton = true, }: LimitWarningProps) {
    ...
    const percentage = type === "emails" && limit === "unlimited"
        ? 0
    ...
    return (<Alert className={`${isAtLimit ? "bg-red-50 border-red-200" : "bg-amber-50 border-amber-200"} ${className}`}>
      ...
      <AlertDescription className="flex items-center justify-between w-full">
        ...
        {showButton && (<Link href={`/dashboard/settings?tab=subscription&feature=${feature}`}>
            <Button size="sm" variant={isAtLimit ? "destructive" : "outline"} className="ml-4">
              Upgrade Plan
            </Button>
          </Link>)}
      </AlertDescription>
...
export function SubscriberLimitWarning({ artist, currentCount, className = "", showButton = true, }: Omit<LimitWarningProps, "type">) {
    return (<LimitWarning artist={artist} type="subscribers" currentCount={currentCount} className={className} showButton={showButton}/>);
}
export function EmailLimitWarning({ artist, currentCount, className = "", showButton = true, }: Omit<LimitWarningProps, "type">) {
    return (<LimitWarning artist={artist} type="emails" currentCount={currentCount} className={className} showButton={showButton}/>);
}
