...
export function LimitWarning({ artist, type, currentCount, className = '', showButton = true }: LimitWarningProps) {
  ...
  const percentage = limit === 'unlimited' ? 0 : Math.min(100, Math.round((currentCount / (limit as number)) * 100));
  
  // Only show warning if approaching limit (80% or more)
  if (limit === 'unlimited' || percentage < 80) {
    return null;
  }
  ...
  return (
    <Alert className={`${isAtLimit ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'} ${className}`}>
      ...
      <AlertDescription className="flex items-center justify-between w-full">
        <div>
          <p className={`font-medium ${isAtLimit ? 'text-red-800' : 'text-amber-800'}`}>
            ...
            {' '}
            {currentCount.toLocaleString()} / {limit === 'unlimited' ? 'Unlimited' : limit.toLocaleString()}
            {' '}
