...
export function generateEnhancedEmailHtml({ title, previewText, content, subscriptionPlan = 'starter' }: {
    title: string;
    previewText?: string;
    content: string;
...
function processMailyContent(content: string): string {
    ...
    Object.entries(classToStyleMap).forEach(([className, inlineStyle]) => {
        ...
        processedContent = processedContent.replace(classRegex, (match) => {
            const elementMatch = processedContent.substring(processedContent.lastIndexOf('<', processedContent.indexOf(match)), processedContent.indexOf('>', processedContent.indexOf(match)) + 1);
    ...
    processedContent = processedContent.replace(/<img([^>]*?)>/g, (match, attributes) => {
