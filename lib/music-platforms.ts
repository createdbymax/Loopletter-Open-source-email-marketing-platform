// Cross-platform music link generation using ISRC/UPC
...
export interface PlatformLink {
  ...
  icon: string;
  color: string;
  iconSvg?: string; // SVG icon for better quality
}
...
class MusicLinkService {
  // Generate platform-specific URLs using Songlink/Odesli API for accurate song URLs
  async generatePlatformLinks(isrc?: string, upc?: string, artistName?: string, trackTitle?: string, spotifyUrl?: string): Promise<PlatformLink[]> {
    ...
    // Try to use Songlink/Odesli API first if we have a Spotify URL
    if (spotifyUrl) {
      try {
    ...
    // Fallback to manual generation with improved URL construction
    // Apple Music
  ...
  // Use Songlink/Odesli API to get accurate platform URLs
  ...
  // Parse Songlink API response to extract platform URLs
  ...
  // Generate a universal link that redirects to user's preferred platform
  generateUniversalLink(spotifyUrl: string): string {
    // This could be enhanced to use services like Songlink/Odesli
    // For now, we'll just return the Spotify URL as the primary link
    return spotifyUrl;
  }
}
