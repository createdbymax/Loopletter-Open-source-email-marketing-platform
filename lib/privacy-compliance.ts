...
import { PrivacyConsent, DataSubjectRequest, DataRetentionPolicy, PrivacySettings, DataBreachIncident, PrivacyComplianceStatus, Fan } from './types';
export class PrivacyComplianceService {
    static async recordConsent(fanId: string, artistId: string, consentData: {
    ...
    static async withdrawConsent(fanId: string, consentType: string, withdrawalMethod: string = 'user_request'): Promise<void> {
    ...
    static async createDataSubjectRequest(email: string, artistId: string, requestData: {
    ...
    static async verifyDataSubjectRequest(requestId: string, verificationToken: string): Promise<boolean> {
        const { data, error } = await supabase
            .from('data_subject_requests')
            .update({
    ...
    static async processDataSubjectRequest(requestId: string, processedBy: string): Promise<any> {
    ...
    static async generateDataExport(email: string, artistId: string): Promise<any> {
        const { data: fan } = await supabase
            ...
            .single();
        if (!fan)
            return { message: 'No data found for this email address' };
    ...
    static async updatePrivacySettings(artistId: string, settings: Partial<PrivacySettings>): Promise<PrivacySettings> {
        try {
            const { data, error } = await supabase
                .from('privacy_settings')
                .upsert({
    ...
    static async updateRetentionPolicy(artistId: string, dataType: string, policy: Partial<DataRetentionPolicy>): Promise<DataRetentionPolicy> {
        try {
            const { data, error } = await supabase
                .from('data_retention_policies')
                .upsert({
    ...
    static async reportDataBreach(artistId: string, breachData: {
    ...
    private static async sendVerificationEmail(email: string, requestId: string, verificationToken: string): Promise<void> {
